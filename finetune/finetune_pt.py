...
# @time: 2023/4/4 14:40
"""
    文件说明：用deepspeed 进行分布式训练
    https://huggingface.co/docs/peft/main/en/task_guides/seq2seq-prefix-tuning#prefix-tuning-for-conditional-generation
...
def print_trainable_parameters(model):
    ...
    for _, param in model.named_parameters():
        ...
        if num_params == 0 and hasattr(param, "ds_numel"): # todo 这个部分不修改应该是无法运行的？
...
def set_args():
    ...
    parser.add_argument('--prefix_projection', type=bool, default=True, help='')  # 为True就是ptuning v2
...
def main():
    args = set_args()
    config = ChatGLMConfig.from_pretrained(args.model_dir)  # todo model, tokenizer, config 肯定是要替换的
    config.pre_seq_len = args.pre_seq_len
    config.prefix_projection = args.prefix_projection   # 当prefix_projection为True时，为P-Tuning-V2方法，在大模型的Embedding和每一层前都加上新的参数；为False时，为P-Tuning方法，仅在大模型的Embedding上新的参数。
    # todo config 这个参数要怎么换到starcoder 中呢？  starcoder 中不存在prefix projection， 而且chatglm 也不存在
    """ 参考一下chatGLM官方微调中的代码
    ...
    model = ChatGLMForConditionalGeneration.from_pretrained(args.model_dir, config=config)  # 用原始的config加载模型
    tokenizer = ChatGLMTokenizer.from_pretrained(args.model_dir)
    model = model.half().cuda()
    if args.prefix_projection:
        model.gradient_checkpointing_enable()

    # conf 是prefix tuning 相关的配置
    ...
    for name, param in model.named_parameters():
        if not any(nd in name for nd in ["prefix_encoder"]):  # todo 管理在哪些层需要添加参数
    ...
    model_engine, optimizer, _, _ = deepspeed.initialize(config=conf,  # conf
                                                         model=model,
